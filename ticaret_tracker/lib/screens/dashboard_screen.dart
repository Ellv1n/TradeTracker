...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final todaySales = transactions.where((t) =>
        t.type == TxType.sale && t.date.year == now.year && t.date.month == now.month && t.date.day == now.day);
    final todaySalesTotal = todaySales.fold<double>(0, (sum, t) => sum + t.totalAmount);
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                StatCard(title: 'Mal sayı', value: '${products.length} növ', color: Colors.blueGrey, icon: Icons.inventory_2, fullWidth: true),
                if (lowStock.isNotEmpty) ...[
                  ...
                  Row(
                    children: [
                      ...
                      Text('Az qalan mallar (${lowStock.length})', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ],
                  ),
                  ...
                  ...lowStock.map((p) => Card(
                        ...
                        child: ListTile(
                          title: Text(p.name),
                          subtitle: Text('Qalıq: ${p.quantity} ${p.unit}'),
                        ),
                      )),
                ],
